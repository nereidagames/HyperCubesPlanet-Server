...
app.use(express.json({ limit: '50mb' })); // Duży limit dla mapy Nexusa
...
app.get('/api/init-database', async (req, res) => {
  ...
  try {
    ...
    // --- NOWA TABELA: NEXUS MAP ---
    // Przechowuje tylko jeden rekord (ID=1) z danymi mapy głównej
    await pool.query(`CREATE TABLE IF NOT EXISTS nexus_map (id INTEGER PRIMARY KEY CHECK (id = 1), map_data JSONB);`);

    res.status(200).send('Baza danych zaktualizowana (dodano nexus_map).');
  } catch (err) {
    console.error(err);
...
// --- NOWE ENDPOINTY: NEXUS ---

// 1. Pobieranie Nexusa (Dostępne dla każdego)
app.get('/api/nexus', async (req, res) => {
    try {
        ...
        if (result.rows.length > 0) {
            ...
            // Jeśli nie ma mapy w bazie, zwróć 404 (klient wtedy wygeneruje domyślną podłogę)
...
// 2. Zapisywanie Nexusa (TYLKO DLA ADMINA)
app.post('/api/nexus', authenticateToken, async (req, res) => {
    // ZABEZPIECZENIE: Zmień 'nixox2' na swój dokładny nick w grze!
    const allowedAdmins = ['admin', 'nixox2']; 
    
    if (!allowedAdmins.includes(req.user.username)) {
        return res.status(403).json({ message: "Brak uprawnień! Tylko admin może edytować Nexus." });
    }
    ...
    try {
        // UPSERT: Jeśli ID=1 istnieje to aktualizuj, jeśli nie to wstaw
        await pool.query(
        ...
        console.error("Błąd zapisu Nexusa:", e);
        res.status(500).json({ message: e.message });
    }
});


// --- RESZTA API (Auth, Skiny, Znajomi...) ---
...
app.get('/api/skins/mine', authenticateToken, async (req, res) => {
    try { const r = await pool.query(`SELECT id, name, thumbnail, owner_id, created_at FROM skins WHERE owner_id = $1 ORDER BY created_at DESC`, [req.user.userId]); res.json(r.rows); } catch (e) { res.status(500).json({ message: e.message }); }
});
app.get('/api/skins/all', authenticateToken, async (req, res) => {
    try { const r = await pool.query(`SELECT s.id, s.name, s.thumbnail, s.owner_id, u.username as creator FROM skins s JOIN users u ON s.owner_id = u.id ORDER BY s.created_at DESC LIMIT 50`); res.json(r.rows); } catch (e) { res.status(500).json({ message: e.message }); }
});
app.get('/api/skins/:id', authenticateToken, async (req, res) => {
    try { const r = await pool.query(`SELECT blocks_data FROM skins WHERE id = $1`, [req.params.id]); if (r.rows.length === 0) return res.status(404).json({ message: 'Nie znaleziono.' }); res.json(r.rows[0].blocks_data); } catch (e) { res.status(500).json({ message: e.message }); }
});
...
app.post('/api/friends/search', authenticateToken, async (req, res) => {
    try { const r = await pool.query(`SELECT id, username, current_skin_thumbnail FROM users WHERE username ILIKE $1 AND id != $2 LIMIT 10`, [`%${req.body.query}%`, req.user.userId]); res.json(r.rows); } catch (e) { res.status(500).json({ message: e.message }); }
});
...
app.post('/api/coins/update', authenticateToken, async (req, res) => {
    try { const r = await pool.query('UPDATE users SET coins = COALESCE(coins, 0) + $1 WHERE id = $2 RETURNING coins', [req.body.amount, req.user.userId]); res.json({ newBalance: r.rows[0].coins }); } catch (e) { res.status(500).json({ message: e.message }); }
});
