...
async function autoMigrate() {
    console.log("[Server] Rozpoczynam migrację bazy danych...");
    try {
        ...
        await addCol('users', 'coins', 'INTEGER DEFAULT 0 NOT NULL');
        await addCol('users', 'current_skin_thumbnail', 'TEXT');
        await addCol('users', 'current_skin_id', 'INTEGER'); 
        await addCol('users', 'owned_blocks', `JSONB DEFAULT '["Ziemia"]'::jsonb`);
        await addCol('users', 'level', 'INTEGER DEFAULT 1');
        await addCol('users', 'xp', 'INTEGER DEFAULT 0');
        ...
        // NOWOŚĆ: Tabela SMILES (Uśmiechy) - do blokady czasowej
        await pool.query(`
        ...
        `);
        // Indeks dla wydajności sprawdzania ostatniego uśmiechu
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_smiles_sender_recipient ON smiles(sender_id, recipient_id);`);
...
// --- UŚMIECHY (NOWOŚĆ) ---
app.post('/api/user/:id/smile', authenticateToken, async (req, res) => {
    const recipientId = parseInt(req.params.id);
    const senderId = req.user.userId;

    if (recipientId === senderId) {
        return res.status(400).json({ message: "Nie możesz wysłać uśmiechu do siebie." });
    }

    try {
        // 1. Sprawdź cooldown (60 min)
        const lastSmile = await pool.query(`
            SELECT sent_at FROM smiles 
            WHERE sender_id = $1 AND recipient_id = $2 
            ORDER BY sent_at DESC LIMIT 1
        `, [senderId, recipientId]);

        if (lastSmile.rows.length > 0) {
            const lastTime = new Date(lastSmile.rows[0].sent_at).getTime();
            const now = new Date().getTime();
            const diffMinutes = (now - lastTime) / (1000 * 60);

            if (diffMinutes < 60) {
                const minutesLeft = Math.ceil(60 - diffMinutes);
                return res.status(400).json({ message: `Musisz poczekać jeszcze ${minutesLeft} min.` });
            }
        }

        // 2. Zapisz w historii
        await pool.query(`INSERT INTO smiles (sender_id, recipient_id) VALUES ($1, $2)`, [senderId, recipientId]);

        // 3. Pobierz dane nadawcy dla powiadomienia
        const senderInfo = await pool.query(`SELECT username, current_skin_thumbnail FROM users WHERE id = $1`, [senderId]);
        const senderName = senderInfo.rows[0].username;
        const senderThumb = senderInfo.rows[0].current_skin_thumbnail;

        // 4. Dodaj nagrodę (News typu 'smile')
        await addNews(recipientId, 'smile', senderId, null, 'Uśmiech', senderThumb, 1500, 0);

        res.json({ success: true, message: "Wysłano uśmiech!" });
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const userRes = await pool.query(
            `INSERT INTO users (username, password_hash, coins, owned_blocks, level, xp, total_xp) 
             VALUES ($1, $2, 0, '["Ziemia"]'::jsonb, 1, 0, 0) RETURNING id`, 
            [username, hash]
        );
        ...
        if (starterSkin) {
            ...
            const skinRes = await pool.query(
                ...
                [userId, skinName, blocks, thumb]
            );
            const skinId = skinRes.rows[0].id;

            await pool.query(
                `UPDATE users SET current_skin_id = $1, current_skin_thumbnail = $2 WHERE id = $3`, 
                [skinId, thumb, userId]
            );
        }
...
app.post('/api/login-map', authenticateToken, async (req, res) => {
    const { blocks } = req.body;
    try {
...
app.post('/api/user/equip', authenticateToken, async (req, res) => { const { skinId, thumbnail } = req.body; const userId = req.user.userId; try { const check = await pool.query('SELECT id FROM skins WHERE id = $1 AND owner_id = $2', [skinId, userId]); if (check.rows.length === 0) return res.status(403).json({ message: "Nie posiadasz tego skina." }); await pool.query(`UPDATE users SET current_skin_id = $1, current_skin_thumbnail = $2 WHERE id = $3`, [skinId, thumbnail, userId]); res.json({ success: true }); } catch (e) { res.status(500).json({ message: "Błąd bazy." }); } });
app.get('/api/user/profile/:username', authenticateToken, async (req, res) => { try { const { username } = req.params; const r = await pool.query('SELECT id, username, level, created_at, current_skin_thumbnail FROM users WHERE username = $1', [username]); if (r.rows.length === 0) return res.status(404).json({ message: "Gracz nie znaleziony" }); res.json(r.rows[0]); } catch (e) { res.status(500).json({ message: "Błąd serwera" }); } });
app.get('/api/user/:id/wall', authenticateToken, async (req, res) => { const targetUserId = req.params.id; try { const [skins, worlds, prefabs, parts] = await Promise.all([ pool.query(`SELECT s.id, s.name, s.thumbnail, s.created_at, s.owner_id, u.username as creator, u.level as "creatorLevel", u.current_skin_thumbnail as "creatorThumbnail", (SELECT COUNT(*) FROM skin_likes WHERE skin_id = s.id) as likes, (SELECT COUNT(*) FROM skin_comments WHERE skin_id = s.id) as comments FROM skins s JOIN users u ON s.owner_id = u.id WHERE s.owner_id = $1 ORDER BY s.created_at DESC LIMIT 20`, [targetUserId]), pool.query(`SELECT w.id, w.name, w.thumbnail, w.created_at, w.owner_id, u.username as creator, u.level as "creatorLevel", u.current_skin_thumbnail as "creatorThumbnail", 0 as likes, 0 as comments FROM worlds w JOIN users u ON w.owner_id = u.id WHERE w.owner_id = $1 ORDER BY w.created_at DESC LIMIT 20`, [targetUserId]), pool.query(`SELECT p.id, p.name, p.thumbnail, p.created_at, p.owner_id, u.username as creator, u.level as "creatorLevel", u.current_skin_thumbnail as "creatorThumbnail", (SELECT COUNT(*) FROM prefab_likes WHERE prefab_id = p.id) as likes, (SELECT COUNT(*) FROM prefab_comments WHERE prefab_id = p.id) as comments FROM prefabs p JOIN users u ON p.owner_id = u.id WHERE p.owner_id = $1 ORDER BY p.created_at DESC LIMIT 20`, [targetUserId]), pool.query(`SELECT h.id, h.name, h.thumbnail, h.created_at, h.owner_id, u.username as creator, u.level as "creatorLevel", u.current_skin_thumbnail as "creatorThumbnail", (SELECT COUNT(*) FROM part_likes WHERE part_id = h.id) as likes, (SELECT COUNT(*) FROM part_comments WHERE part_id = h.id) as comments FROM hypercube_parts h JOIN users u ON h.owner_id = u.id WHERE h.owner_id = $1 ORDER BY h.created_at DESC LIMIT 20`, [targetUserId]) ]); res.json({ skins: skins.rows, worlds: worlds.rows, prefabs: prefabs.rows, parts: parts.rows }); } catch (e) { res.status(500).json({ message: "Błąd pobierania ściany" }); } });
...
app.delete('/api/friends/:id', authenticateToken, async (req, res) => { const targetId = parseInt(req.params.id); const myId = req.user.userId; try { const r = await pool.query(`DELETE FROM friendships WHERE (user_id1 = $1 AND user_id2 = $2) OR (user_id1 = $2 AND user_id2 = $1)`, [myId, targetId]); if (r.rowCount === 0) return res.status(404).json({ message: "Nie jesteście znajomymi." }); res.json({ success: true, message: "Usunięto ze znajomych." }); [myId, targetId].forEach(pid => { const p = players.get(pid); if (p && p.ws.readyState === 1) { p.ws.send(JSON.stringify({ type: 'friendStatusChange' })); } }); } catch (e) { res.status(500).json({ message: "Błąd serwera" }); } });
