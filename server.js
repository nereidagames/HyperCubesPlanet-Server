...
// Konfiguracja CORS (dostęp z każdego miejsca)
app.use(cors({ origin: '*' })); 
...
app.get('/api/skins/all', authenticateToken, async (req, res) => { 
    try { 
        const query = `
            SELECT 
                s.id, s.name, s.thumbnail, s.owner_id, s.created_at,
    ...
    try { 
        const query = `
            SELECT 
                ...
                (SELECT COUNT(*) FROM skin_likes sl WHERE sl.skin_id = s.id) as likes,
                (SELECT COUNT(*) FROM skin_comments sc WHERE sc.skin_id = s.id) as comments
            FROM skins s JOIN users u ON s.owner_id = u.id
            WHERE s.owner_id = $1 
...
app.get('/api/prefabs/all', authenticateToken, async (req, res) => {
    try {
        const query = `
            SELECT 
                p.id, p.name, p.thumbnail, p.owner_id, p.created_at,
                u.username as creator, u.level as "creatorLevel", u.current_skin_thumbnail as "creatorThumbnail",
    ...
    try {
        const query = `
            SELECT 
                ...
                (SELECT COUNT(*) FROM prefab_likes pl WHERE pl.prefab_id = p.id) as likes,
                (SELECT COUNT(*) FROM prefab_comments pc WHERE pc.prefab_id = p.id) as comments
            FROM prefabs p JOIN users u ON p.owner_id = u.id
    ...
    try {
        const query = `
            SELECT 
                ...
                (SELECT COUNT(*) FROM part_likes pl WHERE pl.part_id = p.id) as likes,
                (SELECT COUNT(*) FROM part_comments pc WHERE pc.part_id = p.id) as comments
            FROM hypercube_parts p JOIN users u ON p.owner_id = u.id
            WHERE p.owner_id = $1 ORDER BY p.created_at DESC
        `;
        const r = await pool.query(query, [req.user.userId]);
        res.json(r.rows);
