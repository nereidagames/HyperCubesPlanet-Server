...
app.post('/api/skins', authenticateToken, async (req, res) => {
    ...
    const userId = req.user.userId;
    try {
        const result = await pool.query(`INSERT INTO skins (owner_id, name, blocks_data, thumbnail) VALUES ($1, $2, $3, $4) RETURNING id`, [userId, name, JSON.stringify(blocks), thumbnail]);
        res.status(201).json({ message: 'Skin zapisany.', skinId: result.rows[0].id });
    } catch (err) { res.status(500).json({ message: 'Błąd.' }); }
});
app.get('/api/skins/mine', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    try { const result = await pool.query(`SELECT id, name, thumbnail, owner_id, created_at FROM skins WHERE owner_id = $1 ORDER BY created_at DESC`, [userId]); res.json(result.rows); } catch (err) { res.status(500).json({ message: 'Błąd.' }); }
});
...
app.get('/api/skins/:id', authenticateToken, async (req, res) => {
    ...
    try { const result = await pool.query(`SELECT blocks_data FROM skins WHERE id = $1`, [skinId]); if (result.rows.length === 0) return res.status(404).json({ message: 'Nie znaleziono.' }); res.json(result.rows[0].blocks_data); } catch (err) { res.status(500).json({ message: 'Błąd.' }); }
});
...
app.post('/api/friends/search', authenticateToken, async (req, res) => {
    ...
    try { const result = await pool.query(`SELECT id, username, current_skin_thumbnail FROM users WHERE username ILIKE $1 AND id != $2 LIMIT 10`, [`%${query}%`, userId]); res.json(result.rows); } catch (err) { res.status(500).json({ message: 'Błąd.' }); }
});
app.post('/api/friends/request', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (check.rows.length > 0) return res.status(400).json({ message: 'Już jesteście powiązani.' });
...
app.get('/api/messages/:username', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const messages = await pool.query(`SELECT m.id, m.sender_id, u.username as sender_username, m.message_text, m.created_at FROM private_messages m JOIN users u ON m.sender_id = u.id WHERE (sender_id = $1 AND recipient_id = $2) OR (sender_id = $2 AND recipient_id = $1) ORDER BY m.created_at ASC`, [userId, otherUserId]);
        res.json(messages.rows);
    } catch (err) { res.status(500).json({ message: 'Błąd.' }); }
});

// --- WEBSOCKET BROADCAST (NAPRAWIONE!) ---
function broadcast(message, excludePlayerId = null) {
  const messageStr = JSON.stringify(message);
  players.forEach((playerData, playerId) => {
    // Używamy readyState === 1 zamiast .OPEN, bo w bibliotece 'ws' stała OPEN może nie być dostępna
        ...
        ws.on('message', async (message) => {
            try {
                ...
                if (data.type === 'playerReady') {
                    ...
                    // POPRAWKA: Dodano skinData do obiektu gracza na serwerze, aby nowi gracze je widzieli
