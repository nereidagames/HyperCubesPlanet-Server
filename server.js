...
async function loadNexusMapToMemory() {
    try {
        ...
        if (result.rows.length > 0) {
            ...
            console.log(`[Server] Załadowano Nexusa: ${nexusBlocksCache.length} bloków.`);
        } else {
            nexusBlocksCache = [];
        }
    } catch (e) {
...
// ==========================================
// API ENDPOINTS - WSZYSTKIE ROZWINIĘTE
// ==========================================
...
// --- ZNAJOMI (FRIENDS) - TEGO BRAKOWAŁO LUB BYŁO ZŁE ---
...
app.get('/api/friends', authenticateToken, async (req, res) => {
    try {
        ...
        // Mapowanie statusu online
        const friends = friendsRes.rows.map(f => ({
            ...f,
            isOnline: players.has(f.id)
        }));

        res.json({ friends, requests: requestsRes.rows });
    } catch (e) {
        console.error(e);
        res.status(500).json({ message: e.message });
    }
...
app.post('/api/friends/search', authenticateToken, async (req, res) => {
    try {
        const r = await pool.query(`
            ...
            LIMIT 10
        `, [`%${req.body.query}%`, req.user.userId]);
        res.json(r.rows);
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
app.post('/api/friends/request', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if(t && t.ws.readyState === 1) {
            t.ws.send(JSON.stringify({ type: 'friendRequestReceived', from: req.user.username }));
        }
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
app.post('/api/friends/accept', authenticateToken, async (req, res) => {
    try {
        ...
        if(ms && ms.ws.readyState === 1) {
            ms.ws.send(JSON.stringify({ type: 'friendStatusChange' }));
        }
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
app.get('/api/login-map', async (req, res) => { try { const result = await pool.query('SELECT map_data FROM login_map WHERE id = 1'); if (result.rows.length > 0) return res.json(result.rows[0].map_data || []); res.json([]); } catch (e) { res.status(500).json({ message: e.message }); } });
app.post('/api/login-map', authenticateToken, async (req, res) => { const { blocks } = req.body; try { await pool.query(`INSERT INTO login_map (id, map_data) VALUES (1, $1) ON CONFLICT (id) DO UPDATE SET map_data = $1`, [JSON.stringify(blocks)]); res.json({ message: 'Mapa logowania zapisana!' }); } catch (e) { res.status(500).json({ message: e.message }); } });
app.get('/api/starter-skins', async (req, res) => { try { const r = await pool.query('SELECT * FROM starter_skins ORDER BY id ASC'); res.json(r.rows); } catch (e) { res.status(500).json({ message: e.message }); } });
app.post('/api/starter-skins', authenticateToken, async (req, res) => { const { name, blocks, thumbnail } = req.body; try { await pool.query(`INSERT INTO starter_skins (name, blocks_data, thumbnail) VALUES ($1, $2, $3)`, [name, JSON.stringify(blocks), thumbnail]); res.json({ message: 'Skin startowy zapisany!' }); } catch (e) { res.status(500).json({ message: e.message }); } });
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => { 
        ...
        setTimeout(() => { 
            if (ws.readyState === ws.OPEN) { 
                ...
                const nexusPlayers = []; 
                players.forEach((p, id) => { if (id !== playerId && String(p.currentWorld) === 'nexus') { nexusPlayers.push({ id: p.id, nickname: p.nickname, skinData: p.skinData, position: p.position, quaternion: p.quaternion }); } }); 
            ...
            try { 
                ...
                if (data.type === 'joinWorld') { 
                    ...
                    if (oldWorld !== newWorld) { 
                        ...
                        const roomPlayers = []; 
                        players.forEach((other, oid) => { if (oid !== playerId && String(other.currentWorld) === newWorld) { roomPlayers.push({ id: other.id, nickname: other.nickname, skinData: other.skinData, position: other.position, quaternion: other.quaternion }); } }); 
